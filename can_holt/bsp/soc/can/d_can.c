...
)
{
  ...
  if (((PendingIntr & d_CAN_IXR_ERROR_MASK) != (Uint32_t)0) && (d_CAN_Config[channel].errorHandler != NULL))
  {
...
/*********************************************************************/ /**
   <!-- d_CAN_ProgramSingleIDFilter -->

   Program a single ID acceptance filter.
 *************************************************************************/
d_Status_t
d_CAN_ProgramSingleIDFilter(
    ...
    const d_CAN_Message_t *const pMessage)
{
  ...
  d_Status_t status = d_STATUS_SUCCESS;
  Uint32_t value = d_CAN_CreateIdValue(pMessage->id, pMessage->substituteRemoteTxRequest,
                                       pMessage->extended, pMessage->exId, pMessage->remoteTxRequest);
  ...
  if (status == d_STATUS_SUCCESS)
  {
    ...
    /*Configure the filter mask for extended frames. Write FFFF_FFFFh to the can.AFMR register.*/
    /*  Note: This value is valid only for single ID filter configurations */
    status = d_CAN_RegisterWrite(channel, d_CAN_REGISTER_AFMR1_OFFSET, 0xFFFFFFFFu);

    if (status == d_STATUS_SUCCESS)
    {
      /* Configure the filter ID for extended frames. Write 32-bit arbitration field value computed above
        to the can.AFIR register.*/
      status = d_CAN_RegisterWrite(channel, d_CAN_REGISTER_AFIR1_OFFSET, value);
      /* Disable acceptance filters. Write a 0 to the can.AFR register.*/
      d_CAN_RegisterWrite(channel, d_CAN_REGISTER_AFR_OFFSET, 1);
    }
    else
    {
      /* return status error */
...
d_Status_t
d_CAN_ProgramRangeIDFilter(
    const Uint32_t channel,
    const d_CAN_Message_t *const pCanIdStart,
	const d_CAN_Message_t *const pCanIdEnd)
{

  d_Status_t status = d_STATUS_SUCCESS;
  Uint32_t can_id_start = d_CAN_CreateIdValue(pCanIdStart->id, pCanIdStart->substituteRemoteTxRequest,
		  pCanIdStart->extended, pCanIdStart->exId, pCanIdStart->remoteTxRequest);

  Uint32_t can_id_end = d_CAN_CreateIdValue(pCanIdEnd->id, pCanIdEnd->substituteRemoteTxRequest,
		  pCanIdEnd->extended, pCanIdEnd->exId, pCanIdEnd->remoteTxRequest);

  Uint32_t mask = ~(can_id_start ^ can_id_end);
  Uint32_t filter = (can_id_start & can_id_end);
  ...
  if (status == d_STATUS_SUCCESS)
  {
    ...
    /*Configure the filter mask for extended frames. Write FFFF_FFFFh to the can.AFMR register.*/
    /*  Note: This value is valid only for single ID filter configurations */
    status = d_CAN_RegisterWrite(channel, d_CAN_REGISTER_AFMR2_OFFSET, mask);

    if (status == d_STATUS_SUCCESS)
    {
      /* Configure the filter ID for extended frames. Write 32-bit arbitration field value computed above
        to the can.AFIR register.*/
      status = d_CAN_RegisterWrite(channel, d_CAN_REGISTER_AFIR2_OFFSET, filter);
      /* Disable acceptance filters. Write a 0 to the can.AFR register.*/
      d_CAN_RegisterWrite(channel, d_CAN_REGISTER_AFR_OFFSET, 3);
    }
    else
    {
      /* return status error */
